...
export function ProjectCarousel({ children }: ProjectCarouselProps) {
  ...
  return (
    <div className="bg-netbook flex h-[370px] w-[640px] bg-cover px-[105px] pb-[96px] pt-[16px]">
      <div className="pointer-events-auto relative flex grow items-center justify-between overflow-hidden bg-background">
        ...
        {shouldShowButtons && (
          <Button
            variant="outline"
            className="absolute left-2  rounded-full px-2"
            onClick={() => moveIndex(-1)}
          >
            <ChevronLeft className=" h-6 w-6 shrink-0 transition-transform duration-200" />
          </Button>
        )}
        {shouldShowButtons && (
          <Button
            variant="outline"
            className="absolute right-2 rounded-full px-2"
            onClick={() => moveIndex(1)}
          >
